...
class PairwiseRankRunner2(gym.Env):
    def __init__(self, reward_val=0.5, low_reward=0, high_reward=1, mid_low_reward = 0):
        super(PairwiseRankRunner2, self).__init__()
        
        
        excel = pd.read_csv("normalised_5_log_listwise.csv")
        column_names = ['Cycle_normalized', 'Duration_normalized', 'last_run_normalised_log', 'times_ran_normalized', 'Failure_Percentage_normalized', 'my_maturity_normalized', 'Month_normalized', 'Rank']
    
        excel = pd.read_csv("all_normed_logged6.csv")
        column_names = ['Cycle', 'Duration', 'last_run', 'times_ran', 'Failure_Percentage', 'my_maturity', 'Month', 'Rank']
    ...
    def reset(self, test = False):
        ...
        st1 = [value for key, value in self.arr[self.list_counter].items() if key != "Rank"]
        st2 = [value for key, value in self.pivot.items() if key != "Rank"]
    ...
    def step(self, action):
        ...
        ob1 = [value for key, value in self.arr[self.list_counter].items() if key != "Rank"]
        ob2 = [value for key, value in self.pivot.items() if key != "Rank"]
