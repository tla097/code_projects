...
from pairwise_rank_env_2 import PairwiseRankRunner2
...
class Runner():
    ...
    def __init__(self, tv = 0, ask= 0, mini_batch=1, added_comparison= 1, comparison= "Duration", layers =1, activation= "relu", load = "", save= "test", des= "des", results = "res", new_load = "n", gamma = 0.99, reward = 0.5, eps_decay = 10000, lr = 1e-4, low_reward = 0, high_reward = 1, mid_low_reward = 0, test_train = "test", batch_size = 128, mem_size = 10000, tau = 0.005, start= 0.9):
        ...
        self.env = PairwiseRankRunner2(reward_val=reward, low_reward=low_reward, high_reward=high_reward, mid_low_reward = mid_low_reward)
        ...
        self.folder_path = "RL/rlq/models/pairwise_rank_2/_"
    ...
    def test(self, mid_run= True, k= -1, stage = ""):
        ...
        if not mid_run:
            ...
            self.folder_path = "RL/rlq/models/pairwise_rank_2/_" + self.SAVE
    ...
    def train(self):
        ...
        if new_load == "load":
            self.target_net.load_state_dict(torch.load("RL/rlq/models/pairwise_rank_2/_" + self.LOAD + "/target_" + self.LOAD + ".pth"))
            self.target_net.to(self.device)
            self.target_net.train()

            self.policy_net.load_state_dict(torch.load("RL/rlq/models/pairwise_rank_2/_" + self.LOAD + "/policy_" + self.LOAD + ".pth"))
            self.policy_net.to(self.device)
            self.policy_net.train()
            
            
            self.optimizer.load_state_dict(torch.load("RL/rlq/models/pairwise_rank_2/_" + self.LOAD + "/optimiser_" + self.LOAD + ".pth"))
