...
from listwise_env_truncation_trial import ListwiseEnv
...
class Runner():
    ...
    def __init__(self, tv = 0, ask= 0, mini_batch=1, added_comparison= 1, comparison= "Duration", layers =1, activation= "relu", load = "", save= "test", des= "des", results = "res", new_load = "new", gamma = 0.99, reward = 0.5, eps_decay = 10000, lr = 1e-4, test_train="test", num_actions= 10):
        ...
        self.env = ListwiseEnv(num_actions=num_actions)
        ...
        self.folder_path = "RL/rlq/models/list_wise/_"
        ...
        self.test_env = ListwiseEnv(num_actions= self.num_actions)
    ...
    def test(self, mid_run= True, k= -1, stage = ""):
        ...
        state = torch.tensor(self.test_env.reset(test=True), dtype=torch.float32, device=self.device).unsqueeze(0)
        ...
        while True:
            ...
            state, reward, done, truncated = self.test_env.step(action, test = True)
        ...
        string2 = f"reward = {correct/self.num_actions} - {truncated}"
