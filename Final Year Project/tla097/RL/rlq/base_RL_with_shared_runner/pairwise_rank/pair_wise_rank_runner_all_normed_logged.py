...
from pair_wise_rank_env_all_normed_logged import PairWiseRankEnv
...
class PairWiseRankRunner():
    
    def __init__(self, tv = 0, ask= 0, mini_batch=1, added_comparison= 1, comparison= "Duration", layers =1, activation= "relu", load = "", save= "test", des= "des", results = "res", new_load = "n", gamma = 0.99, reward = 0.5, eps_decay = 10000, lr = 1e-4, low_reward = 0, high_reward = 1, mid_low_reward = 0, test_train = "test", batch_size = 128, mem_size = 10000, tau = 0.005, start= 0.9, test_frequency = 20, test_length = 50):
        ...
        self.env = PairWiseRankEnv(reward_val=reward, low_reward=low_reward, high_reward=high_reward, mid_low_reward = mid_low_reward)
        ...
        self.folder_path = "RL/rlq/models/pairwise/_"
    ...
    def run(self):
        ...
        if self.test_train == "test":
            self.test(False, num_test_rounds=self.test_length)
        else:
            print("train")
            exit = self.train()
    ...
    def trial(self):
        ...
        for rounds in range(1, 10 + 1):
            ...
            while True: 
                ...
                if done:
                    with open(self.folder_path + "/_" + self.TRAINING_RESULTS + ".txt", 'a') as f:
    ...
    def test(self, mid_run= True, k= -1, stage = 0, num_test_rounds = 100, round_steps = 0):
        print(num_test_rounds)
        print(mid_run)
        ...
        if not mid_run:
            ...
            self.folder_path = "RL/rlq/models/pair_wise_rank/_" + self.SAVE
            ...
            with open("picklenet.pkl", "rb") as p:
                self.policy_net = pickle.load(p)
            with open("post_dict.txt", "w") as f:
        ...
        state = torch.tensor(self.env.reset(arr_len=num_test_rounds, test=True), dtype=torch.float32, device=self.device).unsqueeze(0)
        print(f"Testing now: file = {self.SAVE} with arr len {num_test_rounds}\n")
        ...
        mae, inversions = self.env.calculate_accuracy()
        npra = self.env.calc_normalised_rank_point_average()
        afpd = self.env.calc_APFD()
        
        string1 = f"\nROUND {stage} - total steps = {self.total_steps} - {self.SAVE} - round steps = {round_steps}   "
        string2 = f""
        string3 = f"correct = {correct/counter:.5f} - mae = {mae}, AFPD = {afpd}, NPRA = {npra}"
        string4 = ""
    
        self.prev_tests.append(correct/counter)
        exit = False
        l = len(self.env.arr)
        if self.eps_threshold <= 0.06:
            if (self.previous_average >= 0.95):
                string4  = "\n\n\nCORRECTNESS THRESHOLD REACHED\n\n\n\n\n"
                exit = True
            elif (stage >= l * math.log(l, 2)):
                if len(self.prev_tests) == 5:
                    self.previous_average = sum(self.prev_tests)/len(self.prev_tests)
                    current_average = correct/counter
                    self.prev_tests.append(current_average)
                    
                    if current_average <= self.previous_average:
                        string4  = "\n\n\nAVERAGE THRESHOLD REACHED\n\n\n\n\n"
                        exit = True
        ...
        with open(self.folder_path + "/results/_" + self.TRAINING_RESULTS + ".txt", 'a') as f:
            f.write(f"{string2 + string3 + string4}")
            f.write(f"time taken = {time_taken:.4f}")
    ...
    def train(self):
        ...
        if new_load == "load":
            self.target_net.load_state_dict(torch.load("RL/rlq/models/pair_wise_rank/_" + self.LOAD + "/target_" + self.LOAD + ".pth"))
            self.target_net.to(self.device)
            self.target_net.train()
            ...
            self.policy_net.load_state_dict(torch.load("RL/rlq/models/pair_wise_rank/_" + self.LOAD + "/policy_" + self.LOAD + ".pth"))
            self.policy_net.to(self.device)
            self.policy_net.train()
            ...
            self.optimizer.load_state_dict(torch.load("RL/rlq/models/pair_wise_rank/_" + self.LOAD + "/optimiser_" + self.LOAD + ".pth"))
        ...
        k = 0
        keep = 0
        self.test(True, stage=0, num_test_rounds=self.test_length, round_steps= k)
        for rounds in range(1, self.num_rounds + 1):
            ...
            if rounds % self.test_frequency == 0:
                exit = self.test(True, stage=rounds, num_test_rounds=self.test_length, round_steps=k)
            k = 0

            
            # if rounds % 20 == 0:
            #     self.test(True, stage=rounds, num_test_rounds=50)
...
def add_thread_f():
    ...
    if ask == "y":
        ...
        if name != "cancel": # and comparison != "cancel":
            ...
            runner = PairWiseRankRunner(layers=layers,  load=load, save=name, des=des, eps_decay=eps, results=res, new_load=nl, gamma=gamma, reward=reward, test_train=test_train)   
            return threading.Thread(target=runner.run, name=str(name))
        else:
            return None
    else:
        runner = PairWiseRankRunner(save=name)   
        return threading.Thread(target=runner.run, name=str(name))
...
# eps = 10000
# test_frequency = 20
# while num_test_files != 1000:
#     params = {'eps_decay' : eps, 'test_frequency':test_frequency, 'test_length' : num_test_files}#
#     # name = f"e_{eps}:_tf_{test_frequency}:_tl_{num_test_files}"
#     name = "testtest"
#     runner= PairWiseRankRunner(**params, des=str(params), save=name, test_train="train")
#     mae, _ = runner.run()
