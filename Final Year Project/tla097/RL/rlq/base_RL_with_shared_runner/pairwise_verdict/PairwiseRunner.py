...
class Runner():
    ...
    def __init__(self, tv = 0, ask= 0, mini_batch=1, added_comparison= 1, comparison= "Duration", layers =1, activation= "relu", load = "", save= "test", des= "des", results = "res", new_load = "n", gamma = 0.99, reward = 0.5, eps_decay = 10000, lr = 1e-4, low_reward = 0, high_reward = 1, mid_low_reward = 0, test_train = "test", batch_size = 128, mem_size = 10000, tau = 0.005, start= 0.9, file = "default", env = "all_normed"):
        ...
        self.env = PairWiseEnv(reward_val=reward, low_reward=low_reward, high_reward=high_reward, mid_low_reward = mid_low_reward, env = env)
        self.n_actions = 2
        self.num_actions = 2
    ...
    def test(self, mid_run= True, k= -1, stage = "", round_steps = 0):
        
        path = self.base_runner.pre_test(self, mid_run)
    ...
    def train(self):
        ...
        self.base_runner.pre_train(self)
        self.y = 0
        ...
        for rounds in range(1, self.num_rounds + 1):
            ...
            if rounds % 20 == 0:
                ...
                self.base_runner.save_models(self)
            while True: 
                k += 1
                self.y +=1
                self.total_steps += 1
                ...
                # print(1)
                # Perform one step of the optimization (on the policy network)
                self.base_runner.optimize_model(self)
