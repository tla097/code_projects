...
from list_wise_runner import Runner, Transition
...
class Evo_runner(Runner):
    ...
    def __init__(self, tv=0, ask=0, mini_batch=1, added_comparison=1, comparison="Duration", layers=1, activation="relu", load="", save="test", des="des", results="res", new_load="new", gamma=0.99, reward=0.5, eps_decay=10000, lr=0.0001, test_train="test", memory : Memory_Nodes_Stack= None, name = "test", num_actions= 10, fitness = -100,parent_sd= "", params = {}):
        super().__init__(tv, ask, mini_batch, added_comparison, comparison, layers, activation, load, save, des, results, new_load, gamma, reward, eps_decay, lr, test_train, num_actions=num_actions)
    ...
    def run(self):
        ...
        for epoch in range(150):
            ...
            self.env.reset(needed=False, obs=self_observation)
    ...
    def calculate_fitness(self, initial_fitness = False, state = None, just_evo = None):
        ...
        if initial_fitness:
            ...
            state = self.envs[150][0]
            self_observation = self.envs[150][1]
            self.env.reset(needed=False, obs=self_observation)
        else:
            state = self.envs[0][0]
            self_observation = self.envs[0][1]
            self.env.reset(needed=False, obs=self_observation)
        ...
        while True:
            ...
            if truncated:
                self.fitness = rewards/counter * counter - self.num_actions
                break
