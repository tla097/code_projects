...
from pair_wise_evolutionary_environment2 import Evolultionary_Environment
# from '\RL\rlq\list_wise_evolution_runner' import Evo_runner


import importlib.util

from pair_wise_evolution_runner import Evo_runner
...
class EvolutionaryEnvTester(Evolultionary_Environment):
    ...
    def initialise_population(self, pop_size):
        ...
        for k in range(pop_size):
            ...
            self.population[k] = EvoPairWiseTester(test_train="train", save=self.SAVE, num_actions=self.num_actions, random_mutation_rate=self.random_mutation_rate, random_mutation_rate_threshold=self.random_mutation_rate_threshold, uniform_low=self.uniform_low, uniform_high=self.uniform_high)
...
class EvoPairWiseTester(Evo_runner):
...
def availability():
    ...
    for ns in list_num_samples:
        for ne in list_num_events:
            i += 1
            params = {"mutation_type":"availability", "num_samples":int(ns), "num_events":int(ne),"SAVE":"availablility_pairwise"+str(i), "max_generations":20,  "thread" : "availability_mutate", "thread" : "availability"}
...
def random_mutate1():  
    ...
    for lrmrt in list_random_mutation_rate_threshold:
        for lul, luh in list_uniform:
                i += 1
                params = {"mutation_type":"random","random_mutation_rate_threshold":float(lrmrt), "uniform_low":float(lul), "uniform_high":float(luh), "SAVE":"random_mutate1_pairwise" + str(i), "max_generations":20, "thread" : "random1"}
                runner = EvolutionaryEnvTester(**params)
                description = str(params)
                runner.run(description)
...
def random_mutate2():   
    ...
    for lrmrt in list_random_mutation_rate_threshold:
        for lul, luh in list_uniform:
            i += 1
            params = {"mutation_type":"random", "random_mutation_rate_threshold":float(lrmrt), "uniform_low":float(lul), "uniform_high":float(luh), "SAVE":"random_mutate2_pairwise" + str(i), "max_generations":20, "thread" : "random2"}
            runner = EvolutionaryEnvTester(**params)
            description = str(params)
            runner.run(description)
...
runners = []
runner = EvolutionaryEnvTester(SAVE="pairwise_just_evo" , crossover_type="just_evo", mutation_type=None, random_mutation_rate=1, random_mutation_rate_threshold=0.5, uniform_low=0.8, uniform_high=1.2, just_evo = None, num_events=20, num_samples=100)
description = " samples 100 event 20"
runner.run(description)
pairwise1 = threading.Thread(target=runner.run, args=description)
# runners.append(pairwise1)

runner = EvolutionaryEnvTester(SAVE="pairwise_just_cca" , crossover_type="cca", mutation_type= None, random_mutation_rate=1, random_mutation_rate_threshold=0.5, uniform_low=0.8, uniform_high=1.2, just_evo = None, num_events=20, num_samples=100)
description = " samples 100 event 20"
runner.run(description)

pairwise2 = threading.Thread(target=runner.run, args=description)
runners.append(pairwise2)


runner = EvolutionaryEnvTester(SAVE="pairwise_correlation" , crossover_type="correlation", mutation_type= None, random_mutation_rate=1, random_mutation_rate_threshold=0.5, uniform_low=0.8, uniform_high=1.2, just_evo = None, num_events=20, num_samples=100)
description = " samples 100 event 20"
runner.run(description)
pairwise3 = threading.Thread(target=runner.run, args=description)
# runners.append(pairwise3)

runner = EvolutionaryEnvTester(SAVE="pairwise_single" , crossover_type="single", mutation_type= None, random_mutation_rate=1, random_mutation_rate_threshold=0.5, uniform_low=0.8, uniform_high=1.2, just_evo = None, num_events=20, num_samples=100)
description = " samples 100 event 20"
runner.run(description)
pairwise4 = threading.Thread(target=runner.run, args=description)
# runners.append(pairwise4)


# runner = EvolutionaryEnvTester(SAVE="pairwise" , crossover_type="single", mutation_type= None, random_mutation_rate=1, random_mutation_rate_threshold=0.5, uniform_low=0.8, uniform_high=1.2, just_evo = None, num_events=20, num_samples=100)
# description = " samples 100 event 20"
# runner.run(description)
# pairwise5 = threading.Thread(target=runner.run, args=description)
# runners.append(pairwise4)
...
# plan for tomorrow - 8am
# do naive
# check resulits and fix if needed
# finsih slides
# plan what to show
